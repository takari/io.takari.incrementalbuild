...
public class ComposableSecurityManagerPolicyTest {
  ...
  public void testExecutorServiceThreadingWithSecurityManager() throws Exception {
    try {
     ...
     List<String> exs1 = ex.invokeAll(Arrays.asList(getWork("not.allowed"), getWork("allowed"), getWork("not.allowed"), getWork("allowed"), getWork("not.allowed"), getWork("allowed"))).stream().map(x -> {
        try {
          return x.get();
  ...
  public void testNetworkAccess() throws Exception {
    ...
    testee.implies(this.getClass().getProtectionDomain(), new SocketPermission("localhost", "connect"));
  }
