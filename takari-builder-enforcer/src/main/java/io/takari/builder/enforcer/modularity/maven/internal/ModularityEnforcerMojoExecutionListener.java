...
public class ModularityEnforcerMojoExecutionListener implements MojoExecutionListener {
  ...
  public void beforeMojoExecution(MojoExecutionEvent event) {
    if (enforce(event)){
      enforcer.setupProjectContext(event.getSession(), event.getProject(),
          state.getSessionConfig(), state.getEnforcerConfig());
    }
  }
  ...
  private boolean enforce(MojoExecutionEvent event) {
    // not enforcing modularity if this mojo execution is not part of the maven lifecycle or if being invoked from m2e
