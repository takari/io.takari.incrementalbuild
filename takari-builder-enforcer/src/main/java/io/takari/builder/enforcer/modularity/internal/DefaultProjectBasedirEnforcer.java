...
public class DefaultProjectBasedirEnforcer implements ProjectBasedirEnforcer {
  ...
  private ModularityEnforcementPolicy unregisterContextPolicy() {
    ModularityEnforcementPolicy policy =
        (ModularityEnforcementPolicy) ComposableSecurityManagerPolicy
  ...
  ModularityEnforcementPolicy getContextPolicy() {
    return (ModularityEnforcementPolicy) ComposableSecurityManagerPolicy
        .getContextPolicy(KEY_CONTEXT);
  }
  ...
  @Inject
  public DefaultProjectBasedirEnforcer(Collection<ProjectsProvider> projectsProviders,
  ...
  public void setupProjectContext(MavenSession session, MavenProject project,
      SessionConfig sessionConfig, EnforcerConfig enforcerConfig) {
    if (!isEnabledForProject(enforcerConfig, project.getArtifactId())) {
      return;
    }
    ...
    if (enforceProject) {
      ...
      PathMatcher.Builder readMatcherBuilder =
          PathMatcher.builder(this.normalizer).addMatcher(this.readMatcher);
      PathMatcher.Builder writeMatcherBuilder =
          PathMatcher.builder(this.normalizer).addMatcher(this.writeMatcher);
      ...
      if (sessionConfig.isWriteByDefault()) { // ALLOW write within project tree - nested projects
                                              // are excluded
      ...
      addExceptions(readMatcherBuilder, session, project,
          enforcerConfig.getReadExceptions(project.getArtifactId()),
          sessionConfig.isAllowBreakingRules(), upstreamProjects);
      addExceptions(writeMatcherBuilder, session, project,
          enforcerConfig.getWriteExceptions(project.getArtifactId()),
          sessionConfig.isAllowBreakingRules(), upstreamProjects);
      ...
      ProjectContext context = new ProjectContext(normalizer, project.getArtifactId(), readMatcher,
          writeMatcher, execIncludes);
      registerContextPolicy(context);
    }
  }

  /**
   * Returns {@code true} if build enforcement is configured for the project (usually via
   * inheritance), otherwise returns {@code false}. This is necessary to support multimodule project
   * where enforcement is enabled for some but not all modules.
   */
  ...
  public void finishProjectContext(String basedir, MavenProject mavenProject,
  ...
  private String interpolate(String input, MavenProject project, MavenProject upstream) {
    return input
        .replace(START + UPSTREAM + ARTIFACT_ID + STOP,
            upstream == null ? "NO_UPSTREAM" : upstream.getArtifactId())
        .replace(START + PROJECT + ARTIFACT_ID + STOP, project.getArtifactId());
  }

  private static final Pattern pattern =
      Pattern.compile("^\\$\\{dependency\\.(\\??[a-zA-Z0-9\\-]*)\\}");
  ...
  private void processExceptions(String relpath, MavenProject project, Set<MavenProject> upstream,
    ...
    if (matcher.find()) {
      ...
      if (toProcess.isEmpty()) {
        log.warn("No upstream project match " + matcher.group(0) + " for project "
            + project.getArtifactId());
      }
      // check here......
      for (MavenProject other : toProcess) {
        addException(builder, other.getBasedir(), interpolate(rest, project, other),
            allowBreakingRules, allowToIgnore);
      }
    } else {
      File basedir = relpath.startsWith("/") ? multiModuleProjectDirectory : project.getBasedir();
      addException(builder, basedir, interpolate(relpath, project, null), allowBreakingRules,
          false); // this could be a
                  // read exclusion
                  // which would be
                  // pointless
    }
  ...
  protected void addExceptions(PathMatcher.Builder builder, MavenSession session,
      MavenProject project, Collection<String> exceptions, boolean allowBreakingRules,
  ...
  /**
   * @param builder aggregator of allowed rules
   * @param basedir part of file path
   * @param relpath rest of file path (may include bs like /../ or /./
   * @param allowBreakingRules if a rule can break strict modularity (aka the maven build is no
   *        longer a Directed Acyclic Graph)
   * @param allowToIgnore whether or not a rule should be ignored if it is a breaking rule (see
   *        allowBreakingRules)
   */
  protected void addException(PathMatcher.Builder builder, File basedir, String relpath,
      boolean allowBreakingRules, boolean allowToIgnore) {
    final String base = normalizer.normalize(basedir.toPath());
    final String path = normalizer.normalize(
        basedir.toPath().resolve(relpath.startsWith("/") ? relpath.substring(1) : relpath));
    ...
    String from = baseOwner != null
        ? "another maven project " + actualOwner + " in subfolder " + baseOwner
        : "a non maven project path " + path;
    if (allowToIgnore && invalidRule) {
      if (baseOwner != null) {
        log.info("In ./.mvn/basedir-enforcer.txt you have a rule " + relpath
            + " on target project at " + basedir + " that tries to access a file from " + from
            + " the rule will be ignored");
      }
      return;
    }
    if (invalidRule && allowBreakingRules) {
      log.warn("In ./.mvn/basedir-enforcer.txt you have a rule " + relpath
          + " on target project at " + basedir + " that tries to access a file from " + from
  ...
  public void setupMavenSession(MavenSession session, SessionConfig sessionConfig) {
    this.normalizer =
        new PathNormalizer(session.getRequest().getMultiModuleProjectDirectory().toPath());
    ...
    // Root folder will be examined when reading contents... exec commands and others will need
    // this.
    ...
    // since permission is given when the read line is greater than OR EQUAL TO the exclude line,
    // this disables read for
    ...
    Arrays.asList("java.endorsed.dirs", "java.ext.dirs", "java.home", "java.io.tmpdir",
        "java.class.path", "java.library.path", // standard jvm props
    ...
    ).stream().map(prop -> System.getProperty(prop))
        .filter(propVal -> propVal != null && !"".equals(propVal)) // ignore
                                                                   // empty
                                                                   // or
                                                                   // unset
                                                                   // properties
        .flatMap(propVal -> Arrays.asList(propVal.split(File.pathSeparator)).stream())
        .filter(path -> path != null && !".".equals(path) && !"".equals(path)) // don't include
                                                                               // empty place or
                                                                               // current
                                                                               // location
        .map(path -> path.endsWith(File.separator + "jre")
    ...
    readMatcherBuilder
        .includePath(session.getRequest().getMultiModuleProjectDirectory().getAbsolutePath());

    // TODO: just adding this here for now to allow this read, but will do this in a cleaner way
    readMatcherBuilder.includePrefix(
        new File(session.getRequest().getMultiModuleProjectDirectory(), ".mvn").getAbsolutePath());
