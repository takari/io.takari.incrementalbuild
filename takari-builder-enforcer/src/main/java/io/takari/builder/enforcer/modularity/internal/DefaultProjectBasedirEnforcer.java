...
public class DefaultProjectBasedirEnforcer implements ProjectBasedirEnforcer {
    ...
    private void registerContextPolicy(ProjectContext context) {
        ComposableSecurityManagerPolicy.registerContextPolicy(
                KEY_CONTEXT, new ModularityEnforcementPolicy(context, (c, v) -> handleViolation(c, v)));
    }

    private ModularityEnforcementPolicy unregisterContextPolicy() {
        ModularityEnforcementPolicy policy =
                (ModularityEnforcementPolicy) ComposableSecurityManagerPolicy.unregisterContextPolicy(KEY_CONTEXT);
        policy.close();
        return policy;
    }
    ...
    @Inject
    public DefaultProjectBasedirEnforcer(
            Collection<ProjectsProvider> projectsProviders, DefaultPlexusContainer plexus) {
    ...
    public void setupProjectContext(
            MavenSession session, MavenProject project, SessionConfig sessionConfig, EnforcerConfig enforcerConfig) {
        ...
        if (enforceProject) {
            ...
            addExceptions(
                    ...
                    sessionConfig.isAllowBreakingRules(),
                    upstreamProjects);
            addExceptions(
                    writeMatcherBuilder,
                    session,
            ...
            ProjectContext context =
                    new ProjectContext(normalizer, project.getArtifactId(), readMatcher, writeMatcher, execIncludes);
    ...
    private String interpolate(String input, MavenProject project, MavenProject upstream) {
        return input.replace(
                        START + UPSTREAM + ARTIFACT_ID + STOP,
    ...
    private void processExceptions(
            ...
            Set<MavenProject> upstream,
            PathMatcher.Builder builder,
            Set<MavenProject> allProjects,
        ...
        if (matcher.find()) {
            ...
            if (ALL_UPSTREAM.equals(matchDependency)) {
                ...
                toProcess = upstream.stream()
                        .filter(p -> matchDependency.equals(p.getArtifactId()))
            ...
            for (MavenProject other : toProcess) {
                addException(
                        builder,
                        other.getBasedir(),
                        interpolate(rest, project, other),
                        allowBreakingRules,
                        allowToIgnore);
            }
        } else {
            ...
            addException(
                    builder,
                    basedir,
                    interpolate(relpath, project, null),
                    allowBreakingRules,
    ...
    public void setupMavenSession(MavenSession session, SessionConfig sessionConfig) {
        this.normalizer = PathNormalizer.createNormalizer(
        ...
        PathMatcher.Builder writeMatcherBuilder =
                PathMatcher.builder(normalizer).excludeRoot();
        ...
        readMatcherBuilder.includePath(
                session.getRequest().getMultiModuleProjectDirectory().getAbsolutePath());
