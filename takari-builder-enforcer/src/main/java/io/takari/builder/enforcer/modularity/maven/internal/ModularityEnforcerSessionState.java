...
class ModularityEnforcerSessionState {
  ...
  ModularityEnforcerSessionState (MavenSession session) {
    ...
    this.enforcerConfig = multimoduleDir != null ? EnforcerConfig.fromFile(
        session.getRequest().getMultiModuleProjectDirectory().toPath().resolve(ENFORCER_CONFIG_FILE_LOCATION)) : EnforcerConfig.empty();
  }
