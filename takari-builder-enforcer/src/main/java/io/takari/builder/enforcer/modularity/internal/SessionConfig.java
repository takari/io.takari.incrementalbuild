...
public class SessionConfig {
  ...
  public SessionConfig(MavenSession session) {
    ...
    isReadByDefault = isAllowBreakingRules && readProperty(session, ALLOW_READ_BY_DEFAULT_PROPERTY_NAME);
    isWriteByDefault = isAllowBreakingRules && readProperty(session, ALLOW_WRITE_BY_DEFAULT_PROPERTY_NAME);
  }
  ...
  public void logStatus(){
    if (disabled || isLogOnly) {
      ...
      if (isAllowBreakingRules) {
        log.info("Modularity Enforcer property "+ALLOW_BREAKING_PROPERTY_NAME+" is true, rules may break modularity: not all modularity errors are displayed");
      }
      if (isReadByDefault) {
        log.info("Modularity Enforcer property "+ALLOW_READ_BY_DEFAULT_PROPERTY_NAME+" is true, non project files will be readable by default: not all read errors are displayed");
      }
      if (isWriteByDefault) {
        log.info("Modularity Enforcer property "+ALLOW_WRITE_BY_DEFAULT_PROPERTY_NAME+" is true, non project files will be writeable by default: not all write errors are displayed");
      }
    } else {
      log.info("Modularity Enforcer is enabled");
      if (isAllowBreakingRules) {
        log.info("Modularity Enforcer property "+ALLOW_BREAKING_PROPERTY_NAME+" is true, rules may break modularity: not all modularity errors are displayed: don't trust this build!!!");
      }
      if (isReadByDefault) {
        log.info("Modularity Enforcer property "+ALLOW_READ_BY_DEFAULT_PROPERTY_NAME+" is true, non project files will be readable by default: don't trust this build!!!");
      }
      if (isWriteByDefault) {
        log.info("Modularity Enforcer property "+ALLOW_WRITE_BY_DEFAULT_PROPERTY_NAME+" is true, non project files will be writeable by default: don't trust this build!!!");
      }
    }
  }
