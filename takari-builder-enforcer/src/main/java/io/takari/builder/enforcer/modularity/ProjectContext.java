...
/**
 * This class contains a list of read and write excludes that must be fully serializable, and should not reference any
 * part of the maven model of the project structure. It will serialize itself through
 * {@link ProjectContext#store(OutputStream)}, and must be loaded, usually by a forked process, in the static method
 * {@link ProjectContext#load(InputStream)}
 *
 * @author igor fed
 * @author rex.hoffman
 */
public class ProjectContext {
  ...
  public ProjectContext(PathNormalizer normalizer, String id, PathMatcher readMatcher, PathMatcher writeMatcher,
  ...
  public final boolean checkExecute(String exec) {
    if (execIncludes(exec)) { return true; }
    return false;
  }
  ...
  boolean execIncludes(String execPath) {
    String exec = execPath.contains("/") ? execPath.substring(execPath.lastIndexOf('/') + 1) : execPath;
