...
public class InputDirectoryFilesTest {
  ...
  public void testMultivalueWithoutLocation_required() throws Exception {
    ...
    builder(temp.getRoot()) //
        .withConfigurationXml("<directories><files><excludes><e>e.txt</e></excludes></files></directories>") //
        .build(_ListDirectoryFilesData.class, "directories");
  ...
  static class _CompileSourceRoots {
    ...
    @InputDirectoryFiles(filesRequired = true, value = { SOURCE_ROOTS_EXPR, "/bad" }, includes = "**/*")
    ...
    @InputDirectoryFiles(filesRequired = true, value = { TEST_SOURCE_ROOTS_EXPR, "/bad" }, includes = "**/*")
  ...
  public void testCompileSourceRoots() throws Exception {
    ...
    CollectionValue input = builder(temp.getRoot()) //
        .withCompileSourceRoot(dir1.getCanonicalPath()).withCompileSourceRoot(dir2.getCanonicalPath())
        .build(_CompileSourceRoots.class, "compileSourceRootFiles");

    assertThat(input.configuration).hasSize(1);
    assertEquals(dir1.getCanonicalPath(), ((IDirectoryFiles)input.configuration.get(0)).location().getCanonicalPath());
  }
  ...
  public void testTestCompileSourceRoots() throws Exception {
    ...
    CollectionValue input = builder(temp.getRoot()) //
        .withTestCompileSourceRoot(dir1.getCanonicalPath()).withTestCompileSourceRoot(dir2.getCanonicalPath())
  ...
  public void testCompileSourceRootsExtraParams() throws Exception {
    thrown.expect(InvalidConfigurationException.class);
    thrown.expectMessage(String.format("%s can not have other values provided along with it", SOURCE_ROOTS_EXPR));
  ...
  public void testTestCompileSourceRootsExtraParams() throws Exception {
    thrown.expect(InvalidConfigurationException.class);
    thrown.expectMessage(String.format("%s can not have other values provided along with it", TEST_SOURCE_ROOTS_EXPR));
  ...
  public void testConfiguredCompileSourceRootsFails() throws Exception {
    thrown.expect(InvalidConfigurationException.class);
    thrown
        .expectMessage(String.format("_Data.files: %s expression is not allowed in configuration", SOURCE_ROOTS_EXPR));
    ...
    builder(temp.getRoot()) //
        .withCompileSourceRoot(dir1.getCanonicalPath()).withCompileSourceRoot(dir2.getCanonicalPath())
        .withConfigurationXml(String.format("<files><files><location>%s</location></files></files>", SOURCE_ROOTS_EXPR))
  ...
  public void testConfiguredTestCompileSourceRootsFails() throws Exception {
    ...
    thrown.expectMessage(
        String.format("_Data.files: %s expression is not allowed in configuration", TEST_SOURCE_ROOTS_EXPR));
    ...
    builder(temp.getRoot()) //
        ...
        .withConfigurationXml(
            String.format("<files><files><location>%s</location></files></files>", TEST_SOURCE_ROOTS_EXPR))
        .build(_Data.class, "files");
  }
}
