...
public class MavenDependencyResolverTest {
    ...
    public void setup(String projectdir) throws Exception {
        ...
        WorkspaceReader reader = new TestWorkspaceReader(Arrays.asList(directProject, transitiveProject));
        ...
        resolver = new MavenDependencyResolver(project, repoSession, repoSystem, new ScopedProjectDependencyCache());
    }
    ...
    private class TestWorkspaceReader implements WorkspaceReader {
        ...
        public File findArtifact(Artifact artifact) {
            MavenProject project = projectsByGAV.get(
    ...
    public void testResolveAllCompileDependencies() throws Exception {
        setup("all-compile");
        Map<IArtifactMetadata, Path> resolved = resolver.getProjectDependencies(true, ResolutionScope.COMPILE);
    ...
    public void testResolveDirectDependencies() throws Exception {
        setup("all-compile");
        Map<IArtifactMetadata, Path> resolved = resolver.getProjectDependencies(false, ResolutionScope.COMPILE);
        assertThat(resolved).containsValue(jarfile.toPath());
        assertThat(resolved).containsValue(dirfiledirect.toPath());
