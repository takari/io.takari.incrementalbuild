...
public class InputDirectoryTest {
  ...
  static class _CompileSourceRoots {
    ...
    @InputDirectory(filesRequired = true, value = {"/bad", TEST_SOURCE_ROOTS_EXPR},
        includes = "**/*")
  ...
  public void testCompileSourceRoots() throws Exception {
    ...
    assertEquals(dir1.getCanonicalPath(), ((InputDirectoryValue) input.configuration.get(0)).location.toRealPath().toString());
  }
  ...
  public void testConfiguredCompileSourceRootsFails() throws Exception {
    ...
    thrown.expectMessage(
        String.format("_Data.directory: %s expression is not allowed in configuration", SOURCE_ROOTS_EXPR));
    ...
    builder(temp.getRoot()) //
        .withCompileSourceRoot(dir1.getCanonicalPath())
        .withCompileSourceRoot(dir2.getCanonicalPath())
        .withConfigurationXml(String.format("<directory><location>%s</location></directory>", SOURCE_ROOTS_EXPR))
  ...
  public void testConfiguredTestCompileSourceRootsFails() throws Exception {
    thrown.expect(InvalidConfigurationException.class);
    thrown.expectMessage(
        String.format("_Data.directory: %s expression is not allowed in configuration", TEST_SOURCE_ROOTS_EXPR));
    ...
    builder(temp.getRoot()) //
        .withCompileSourceRoot(dir1.getCanonicalPath())
        .withCompileSourceRoot(dir2.getCanonicalPath())
        .withConfigurationXml(String.format("<directory><location>%s</location></directory>", TEST_SOURCE_ROOTS_EXPR))
        .build(_Data.class, "directory");
  }
