...
public abstract class AbstractIncrementalMojo extends AbstractMojo {
  ...
  public final void execute() throws MojoExecutionException, MojoFailureException {
    List<Artifact> classpath = __internal_execution.getMojoDescriptor().getPluginDescriptor().getArtifacts();
    ...
    BuilderRunner.create(log, builderType, __internal_execution.getGoal()) //
        ...
        .setSessionProperties(__internal_session.getSystemProperties(), __internal_session.getUserProperties()) //
        ...
        .setClasspath(classpath.stream().map(a -> a.getFile().toPath()).collect(Collectors.toList()), classpathDigester) //
  ...
  private String getBuilderId() {
    ...
    return String.format("%s:%s:%s", descriptor.getGroupId(), descriptor.getArtifactId(), __internal_execution.getGoal());
  }
