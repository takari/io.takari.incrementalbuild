...
public class BuilderWorkspace {
  ...
  private Workspace getWorkspace(Path path) {
    if (path.normalize().startsWith(projectBasedir)) { return workspace; }
    return filesystem;
  }
  ...
  private Set<Path> doWorkspaceWalk(Path basedir) throws IOException {
    ...
    getWorkspace(basedir).walk(basedir.toFile(), new FileVisitor() {
      ...
      public void visit(File file, long lastModified, long length, ResourceStatus status) {
        ...
        if (!getWorkspace(basedir).getResourceStatus(file, lastModified, length).equals(ResourceStatus.REMOVED)) {
  ...
  private Set<Path> getUnchanged(Path basedir, Set<Path> changed) {
    if (oldExecutionState == null) { return Collections.emptySet(); }
    return oldExecutionState.inputsDigest.files().stream().filter(p -> changed == null || !changed.contains(p))
        .filter(p -> getWorkspace(p).isRegularFile(p.toFile()) || getWorkspace(p).isDirectory(p.toFile()))
