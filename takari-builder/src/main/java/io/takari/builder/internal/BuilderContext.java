...
public class BuilderContext {
  ...
  public void leave() {
    ...
    if (policy.getBuilderContext() != this) { throw new IllegalStateException(); }
  ...
  static class BuilderContextPolicy implements Policy {
    ...
    public void checkSocketPermission() {
      checkScope();
      if (!ctx.checkSockets()) { throw new SecurityException(); }
    }

    @Override
    public void checkRead(String file) {
      checkScope();
      if (readPrivileged.get() == Boolean.TRUE) { return; }
      try {
    ...
    public void checkPropertyPermission(String action, String name) {
      checkScope();
      if (!ctx.checkAndRecordProperty(action, name)) { throw new SecurityException(); }
    }
    ...
    private void checkScope() {
      if (!inScope.get()) { throw new IllegalStateException("BuilderContext is no longer in scope"); }
    }
  ...
  public static class Builder {
    ...
    public BuilderContext build() {
      ...
      return new BuilderContext(log, normalizer, id, readMatcher, writeMatcher, tempMatcher, messages, execExceptions,
          networkAccessAllowed, readExceptions, writeExceptions, readAndTrackExceptionsMatcher, inprogressWriter,
          workspace);
    }
  }

  public static Builder builder(Logger log, String id, Path sessionBasedir, MessageCollector messages,
  ...
  /*
   * this lock guarantees that "file created by this builder" and "file exists" checks are in sync with each other as
   * observed by checkRead method. failure to synchronize the two checks results in checkRead false negative if
   * "file exists" check becomes true before "created by this builder".
  ...
  private BuilderContext(Logger log, PathNormalizer normalizer, String id, PathMatcher readMatcher,
      PathMatcher writeMatcher, PathMatcher tempMatcher, MessageCollector messages, Collection<String> execExceptions,
      boolean networkAccessAllowed, Collection<String> readExceptions, Collection<String> writeExceptions,
      PathMatcher readAndTrackExceptionsMatcher, InprogressStateWriter inprogressWriter, BuilderWorkspace workspace) {
    this.log = log;
    this.id = id;
  ...
  private static FileMatcher getExceptionsMatcher(Collection<String> exceptions) {
    return exceptions != null && !exceptions.isEmpty() ? FileMatcher.matcher(Paths.get("/"), exceptions, null)
  ...
  public final boolean checkRead(String file) {
    ...
    if (readExceptionsMatcher.matches(normalized)
        || readAndTrackExceptionsMatcher.includes(normalized)) { return true; }

    synchronized (writeLock) {
      if (writes.contains(normalized) || tempWrites.contains(normalized)) { return true; }
  ...
  public final boolean checkAndRecordWrite(String file) {
    ...
    if (writeExceptionsMatcher.matches(normalized)) { return true; }
    ...
    synchronized (writeLock) {
      ...
      if (workspace.isRegularFile(Paths.get(normalized))
          && !readAndTrackExceptionsMatcher.includes(normalized)) { return false; }
  ...
  public boolean checkAndRecordProperty(String action, String name) {
    ...
    if (action.equals(PROPERTY_WRITE_ACTION)) { return true; }
