...
class MessageCollector implements Messages {
    ...
    private void log(Message message) {
        ...
        switch (message.severity) {
            case ERROR:
                log.error("{}:[{},{}] {}", resource, message.line, message.column, message.message, message.cause);
                break;
            case WARNING:
                log.warn("{}:[{},{}] {}", resource, message.line, message.column, message.message, message.cause);
                break;
            default:
                log.info("{}:[{},{}] {}", resource, message.line, message.column, message.message, message.cause);
                break;
        }
    }
    ...
    <E extends Exception> void replayMessages(ExceptionFactory<E> efactory, final List<Message> messages) throws E {
        messages.forEach(message -> collect(message));
        throwExceptionIfThereWereErrorMessages(efactory);
    }

    <E extends Exception> void throwExceptionIfThereWereErrorMessages(ExceptionFactory<E> efactory) throws E {
        synchronized (messages) {
            assertBuildSuccess(efactory, messages);
        }
    }

    private static <E extends Exception> void assertBuildSuccess(
            ExceptionFactory<E> efactory, Collection<Message> messages) throws E {
        int errorCount = 0;
