...
public abstract class AbstractBuildContext {
     ...
     */
    protected Collection<DefaultResourceMetadata<File>> registerInputs(
        ...
        for (Map.Entry<Path, FileMatcher> subdir :
                FileMatcher.createMatchers(basedir.toPath(), includes, excludes).entrySet()) {
        ...
        if (workspace.getMode() == Mode.DELTA) {
            ...
            final FileMatcher absoluteMatcher = FileMatcher.createMatcher(basedir.toPath(), includes, excludes);
            for (ResourceHolder<?> holder : oldState.getResources().values()) {
                if (holder instanceof FileState) {
                    FileState fileState = (FileState) holder;
                    if (!state.isResource(fileState.file)
                            && !deletedResources.contains(fileState.file)
                            && absoluteMatcher.matches(fileState.file)) {
                        result.add(registerNormalizedInput(fileState.file, fileState.lastModified, fileState.length));
                    }
                }
            }
        }
    ...
    protected Collection<DefaultResource<File>> registerAndProcessInputs(
            File basedir, Collection<String> includes, Collection<String> excludes) throws IOException {
        basedir = normalize(basedir);
    ...
    protected <T extends Serializable> T getResourceAttribute(
            DefaultBuildContextState state, Object resource, String key, Class<T> clazz) {
    ...
    protected void addMessage(
            Object resource, int line, int column, String message, MessageSeverity severity, Throwable cause) {
    ...
    public void commit(MessageSinkAdaptor messager) throws IOException {
        ...
        for (File outputFile : state.getOutputs()) {
            if (state.getResource(outputFile) == null) {
                state.putResource(outputFile, newFileState(outputFile, outputFile.lastModified(), outputFile.length()));
            }
        }
        ...
        if (stateFile != null) {
            ...
            log.debug("Stored incremental build state {} ({} ms)", stateFile, System.currentTimeMillis() - start);
        }
            ...
            for (Map.Entry<Object, Collection<Message>> entry : allMessages.entrySet()) {
                ...
                if (!newMessages.containsKey(resource)) {
                    for (Message message : entry.getValue()) {
                        log(resource, message.line, message.column, message.message, message.severity, message.cause);
                    }
                }
            }
        }
